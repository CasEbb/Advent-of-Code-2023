...
class Grid:
    ...
    def draw_grid_hex(
        ...
        for y in (
            range(size["y_min"] - 2, size["y_min"] + size["height"] + 4)
            if show_all
            else self.y_range()
        ):
            if show_all:
                temp_x_range = list(
                    range(size["x_min"] - 4, size["x_min"] + size["width"] * 2 + 8)
                )
                if y % 2 == 0:
            ...
            for x in temp_x_range:
                if show_all or (x, y) in self.grid:
                    ...
                    temp = [
                        (
                            cell_x * (hex_width * hex_size) + x * hex_size,
                            cell_y * (hex_size) + y * hex_size,
                        )
                        for x, y in hex
                    ]
                    ...
                    if (
                        max_x >= -50
                        and max_y >= -50
                        and min_x <= image_width + 50
                        and min_y <= image_height + 50
                    ):
    ...
    def draw_frames(
        ...
        if sys.version_info >= (3, 11):
            ...
            import datetime as datetime_fix

            UTC = datetime_fix.timezone.utc
    ...
    def draw_grid(
        ...
        if show_lines:
            d.rectangle(
                (
                    (border, border),
                    (
                        border + width * (cell_size[0] + 1),
                        border + height * (cell_size[0] + 1),
                    ),
                ),
                LINE_COLOR,
                LINE_COLOR,
        ...
        for x in range(width):
            for y in range(height):
                color = self.grid.get(
                    (x + self.axis_min(0), y + self.axis_min(1)), self.default
                )
                use = True
                if not show_lines:
                    if color == 0:
                ...
                if use:
                    ...
                    d.rectangle(
                        (
                            (
                                border + x * (cell_size[0] + 1) + 1,
                                border + y * (cell_size[1] + 1) + 1,
                            ),
                            (
                                border
                                + x * (cell_size[0] + 1)
                                + cell_size[0]
                                + (0 if show_lines else 1),
                                border
                                + y * (cell_size[1] + 1)
                                + cell_size[1]
                                + (0 if show_lines else 1),
        ...
        if extra_text is not None:
            ...
            for row in extra_text:
                ...
                for part in row.split("|"):
                    swap = not swap
                    d.text(
                        (x, y + (fnt_diff if swap else 0)),
                        part,
                        (255, 255, 255),
                        font=fnt_segment if swap else fnt_source,
                    )
